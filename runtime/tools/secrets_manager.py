...
Día 11: API simplificada con get_secret() + get_felirni_config()
"""
import json
import threading
import time
...
def get_secret(secret_name: str) -> dict:
    """Get secret from AWS Secrets Manager with 15min cache."""
    with _lock:
...
def invalidate(secret_name: str | None = None) -> None:
    """Invalida cache de un secret específico o todos."""
    with _lock:
        if secret_name:
